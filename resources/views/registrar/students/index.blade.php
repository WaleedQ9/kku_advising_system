...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex justify-between items-center">
            <div>
                <h2 class="text-2xl font-bold text-gray-800">{{ __('إدارة تسجيل الطلاب') }}</h2>
                <p class="text-gray-500 text-sm">{{ __('يمكنك البحث عن الطلاب وإلحاق المواد الدراسية لهم') }}</p>
            </div>
            ...
            <div class="bg-kku-primary/10 px-4 py-2 rounded-2xl border border-kku-primary/20">
                ...
                <span class="text-gray-600 text-xs mr-1">{{ __('طالب مسجل في النظام') }}</span>
            </div>
        </div>
            ...
            <form action="{{ route('registrar.students.index') }}" method="GET" class="space-y-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                    <div class="md:col-span-5">
                        <label class="block text-[10px] font-bold text-gray-400 mb-2 mr-1">{{ __('البحث السريع') }}</label>
                        <div class="relative">
                    ...
                    <div class="md:col-span-4">
                        <label class="block text-[10px] font-bold text-gray-400 mb-2 mr-1">{{ __('تصفية حسب القسم') }}</label>
                    ...
                    <div class="col-span-12 md:col-span-3">
                        <label class="block text-[10px] font-bold text-gray-400 mb-2 mr-1">{{ __('حالة الطالب') }}</label>
                        <select name="status"
                            ...
                            <option value="منتظم" {{ request('status') == 'منتظم' ? 'selected' : '' }}>{{ __('منتظم') }}</option>
                            <option value="متعثر" {{ request('status') == 'متعثر' ? 'selected' : '' }}>{{ __('متعثر') }}</option>
                            <option value="خريج" {{ request('status') == 'خريج' ? 'selected' : '' }}>{{ __('خريج') }}</option>
                        </select>
                    </div>
                ...
                <div class="pt-4 border-t border-gray-50 flex flex-wrap items-center justify-between gap-4">
                    <label
                        ...
                        <div class="flex flex-col">
                            <span class="text-xs font-bold text-amber-800">{{ __('إظهار الطلاب الجدد فقط') }}</span>
                            <span class="text-[10px] text-amber-600/70">{{ __('الطلاب الذين لم يتم تسجيل أي ساعات لهم بعد') }}</span>
                        </div>
                    </label>
            ...
            <table class="w-full text-right">
                <thead class="bg-gray-50 border-b border-gray-100">
                    <tr class="text-gray-500 text-xs font-bold uppercase">
                        <th class="px-6 py-4">{{ __('الطالب') }}</th>
                        <th class="px-6 py-4">{{ __('القسم') }}</th>
                        <th class="px-6 py-4">{{ __('المعدل') }}</th>
                        <th class="px-6 py-4">{{ __('الساعات') }}</th>
                        <th class="px-6 py-4">{{ __('الحالة') }}</th>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-gray-400">
                                ...
                                <p>{{ __('لم يتم العثور على طلاب مطابقين للبحث') }}</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
