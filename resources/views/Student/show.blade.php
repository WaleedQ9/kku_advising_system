...
@section('content')
    <div class="space-y-6">
        ...
        {{-- ── Header ── --}}
        <div
            class="flex flex-wrap justify-between items-center gap-4 bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
            <div class="flex items-center gap-4">
                <div
                    class="w-16 h-16 rounded-2xl {{ $student->hasRiskFlags() ? 'bg-red-500' : 'bg-kku-primary' }} text-white flex items-center justify-center text-2xl font-bold shadow-lg">
            ...
            <div class="col-span-12 lg:col-span-4 space-y-5">
                ...
                <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
                    ...
                    <div class="space-y-3">
                        ...
                        <div
                            class="flex justify-between items-center p-3 {{ $student->courses->sum('pivot.absences_count') > 15 ? 'bg-red-50' : 'bg-gray-50' }} rounded-2xl">
                            <span class="text-xs font-bold text-gray-600">{{ __('إجمالي الغيابات') }}</span>
                            <span
                                class="text-lg font-black {{ $student->courses->sum('pivot.absences_count') > 15 ? 'text-red-600' : 'text-gray-700' }}">
                                {{ $student->courses->sum('pivot.absences_count') }}
                            </span>
                        </div>
                        <div
                            class="flex justify-between items-center p-3 {{ $student->academic_status === 'Warning' ? 'bg-amber-50' : 'bg-gray-50' }} rounded-2xl">
                            <span class="text-xs font-bold text-gray-600">{{ __('الحالة الأكاديمية') }}</span>
                            <span
                                class="text-sm font-bold {{ $student->academic_status === 'Warning' ? 'text-amber-600' : 'text-green-600' }}">
                                {{ $student->academic_status === 'Warning' ? __('تحذير') : __('منتظم') }}
                            </span>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="bg-white p-6 rounded-3xl shadow-sm border border-red-100">
                        ...
                        <div class="space-y-2">
                            @foreach ($student->riskFlags->where('is_resolved', false) as $flag)
                                <div
                                    class="flex items-center justify-between p-3 {{ $flag->severity === 'High' ? 'bg-red-50 border border-red-200' : 'bg-amber-50 border border-amber-200' }} rounded-xl">
                                    <div>
                                        <p
                                            class="text-sm font-bold {{ $flag->severity === 'High' ? 'text-red-700' : 'text-amber-700' }}">
                                            {{ $flag->type === 'Low_GPA' ? __('معدل منخفض') : __('غيابات عالية') }}
                                        </p>
                                        <p class="text-[10px] text-gray-400">
                                            {{ $flag->severity === 'High' ? __('خطورة عالية') : __('خطورة متوسطة') }}</p>
                                    </div>
                                    <form method="POST" action="{{ route('flags.resolve', $flag->id) }}">
                                        @csrf
                                        <button type="submit"
                                            class="text-xs font-bold text-gray-400 hover:text-green-600 transition-colors">
                    ...
                    <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
                        ...
                        <div class="space-y-2">
                            @foreach ($student->dropActions as $drop)
                                <div
                                    class="p-3 rounded-xl border {{ $drop->status === 'Completed' ? 'bg-gray-50 border-gray-200' : 'bg-red-50 border-red-200' }}">
                                    <div class="flex justify-between items-start">
                                        <p class="text-sm font-bold text-gray-700">{{ $drop->course->name ?? '—' }}</p>
                                        <span
                                            class="text-[10px] font-bold px-2 py-0.5 rounded-full {{ $drop->status === 'Completed' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
            ...
            <div class="col-span-12 lg:col-span-8 space-y-6">
                            ...
                            <tbody class="divide-y divide-gray-50">
                                @forelse($student->courses as $course)
                                    ...
                                    <tr class="hover:bg-gray-50/70 transition-colors">
                                        ...
                                        <td class="px-5 py-4">
                                            <span
                                                class="px-2 py-0.5 rounded-lg text-[10px] font-bold {{ $course->level_type == 'تخصص' ? 'bg-purple-50 text-purple-600' : 'bg-gray-100 text-gray-600' }}">
                                                {{ $course->level_type }}
                                            </span>
                                        </td>
                                        <td
                                            class="px-5 py-4 font-bold {{ $absences > 10 ? 'text-red-500' : 'text-gray-600' }}">
                                            {{ $absences }}
                                        </td>
                                        ...
                                        <td class="px-5 py-4 text-center">
                                            @if ($percent >= 100)
                                                <span
                                                    class="px-2 py-1 rounded-md text-[10px] font-bold bg-red-100 text-red-600">{{ __('حرمان') }}</span>
                                            @elseif($percent >= 60)
                                                <span
                                                    class="px-2 py-1 rounded-md text-[10px] font-bold bg-amber-50 text-amber-600">{{ __('إنذار') }}</span>
                                            @else
                                                <span
                                                    class="px-2 py-1 rounded-md text-[10px] font-bold bg-green-50 text-green-600">{{ __('منتظم') }}</span>
                                            @endif
                                        </td>
                                        <td class="px-5 py-4 text-center">
                                            @if ($canDrop)
                                                <button
                                                    onclick="openDropModal({{ $course->id }}, '{{ addslashes($course->name) }}')"
                                            ...
                                            @else
                                                <span
                                                    class="text-[10px] text-gray-300">{{ __('استُنفدت المحاولات') }}</span>
                                            @endif
                                        </td>
                                    </tr>
                                @empty
                ...
                <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                    <div class="p-5 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
                        ...
                        <span class="text-xs text-gray-400">{{ $notes->count() }} {{ __('ملاحظة') }}</span>
                    </div>
                    <div class="p-6 space-y-4">
                        @forelse($notes as $note)
                            <div class="relative pr-8 pb-5 border-r-2 border-gray-100 last:border-0 last:pb-0">
                                <div
                                    class="absolute right-[-6px] top-0 w-3 h-3 rounded-full bg-kku-primary border-2 border-white shadow-sm">
                                </div>
                                <div class="bg-gray-50 p-4 rounded-2xl">
                                    <div class="flex justify-between items-start gap-2 mb-2 flex-wrap">
                                        ...
                                        <div class="flex items-center gap-2">
                                            ...
                                            @if ($note->follow_up_required)
                                                <form method="POST"
                                                    action="{{ route('notes.followup.done', $note->id) }}"
                                                    class="inline">
                                                    @csrf
                                                    <button type="submit"
                                    ...
                                    <p class="text-[10px] text-gray-400 mt-2 italic">بواسطة:
                                        {{ $note->user->name ?? 'المرشد الأكاديمي' }}</p>
                                </div>
                            </div>
                        @empty
        ...
        <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-lg z-10 overflow-hidden animate-modal-in">
            ...
            <form action="{{ route('notes.store') }}" method="POST" class="p-6 space-y-4">
                ...
                <div>
                    ...
                    <textarea name="content" rows="4" required minlength="10" placeholder="اكتب ما تم مناقشته مع الطالب..."
    ...
    <div id="dropModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-md z-10 overflow-hidden animate-modal-in">
            ...
            <form action="{{ route('drop.store', $student->id) }}" method="POST" class="p-6 space-y-4">
                ...
                <div>
                    <label class="block text-xs font-bold text-gray-500 mb-1.5">{{ __('سبب الحذف') }} <span
                            class="text-red-500">*</span></label>
                    <textarea name="reason" rows="3" required minlength="10" placeholder="اذكر سبب حذف المادة..."
